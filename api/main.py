...
async def lifespan(app: FastAPI):
    ...
    tts_mode = settings.get("tts", {}).get("mode", "pi5")
    ...
    grammar_corrector = GrammarCorrector(llm=llm)
    greeting_bot = GreetingBot(llm=llm, pi5_base_url=pi5_url, tts_mode=tts_mode)
    chatbot = LLMChatbot(
        llm=llm,
    ...
    intent_router = IntentRouter(llm=llm, pi5_base_url=pi5_url, tts_mode=tts_mode)
